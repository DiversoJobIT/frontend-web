...
import { MOCK_JOBS, CATEGORIES } from "@/lib/utils/mockData/Mock_Jobs";
...
export default function LandingPage() {
  ...
  const filteredJobs = useMemo(() => {
    return MOCK_JOBS.filter((job) => {
      ...
      const matchCategory = !searchCategory || job.category === searchCategory;
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#fafafa] font-sans antialiased text-[#1a1a1a]">
              ...
              {searchJob && (
                <span className="inline-flex items-center gap-1.5 rounded-full bg-slate-100 px-2.5 py-1 text-xs text-gray-600">
                  ...
                  <button
                    ...
                    className="hover:text-red-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0a59a3] rounded-sm"
                    aria-label={`Eliminar filtro: ${searchJob}`}
                  >
              ...
              {searchWhere && (
                <span className="inline-flex items-center gap-1.5 rounded-full bg-slate-100 px-2.5 py-1 text-xs text-gray-600">
                  ...
                  <button
                    ...
                    className="hover:text-red-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0a59a3] rounded-sm"
                    aria-label={`Eliminar filtro: ${searchWhere}`}
                  >
                    <X className="size-3" aria-hidden="true" />
                  </button>
                </span>
              ...
              {searchCategory && (
                <span className="inline-flex items-center gap-1.5 rounded-full bg-slate-100 px-2.5 py-1 text-xs text-gray-600">
                  ...
                  <button
                    ...
                    className="hover:text-red-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0a59a3] rounded-sm"
                    aria-label={`Eliminar filtro: ${searchCategory}`}
                  >
                    <X className="size-3" aria-hidden="true" />
                  </button>
                </span>
                ...
                >
                  ...
                  <div>
                    <div className="flex items-center justify-between gap-4 mb-4">
                      <div
                        className={`flex size-11 items-center justify-center rounded-xl text-white font-bold text-lg shadow-sm ${job.logoBg}`}
                      >
                        {job.logo}
                      </div>
      ...
      {selectedJob && (
        ...
        >
          <div
            className="w-full max-w-lg bg-white rounded-3xl overflow-hidden shadow-2xl border border-gray-100 flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200"
            role="dialog"
            aria-modal="true"
          ...
          >
            ...
            <div className="p-6 border-b border-gray-100 flex items-start justify-between gap-4">
              <div className="flex items-center gap-4">
                <div
                  className={`flex size-12 items-center justify-center rounded-2xl text-white font-bold text-xl shadow-md ${selectedJob.logoBg}`}
                >
                  {selectedJob.logo}
                </div>
                <div>
      ...
      {showSuccessToast && (
        <div
          role="alert"
          className="fixed bottom-6 left-6 z-50 max-w-sm rounded-2xl bg-slate-900 p-4 text-white shadow-2xl border border-slate-800 flex items-start gap-3 animate-in slide-in-from-bottom-5 duration-300"
        >
